...
@override_settings(ROOT_URLCONF='apps.jobs.tests.urls')
class JobListViewTest(WebTest):
    ...
    def testDoesNotDisplayJobsPastSubmission(self):
        response = self.app.get(reverse('jobs-list'))
        self.assertTrue(self.kitten.title not in response)

    def testDisplaysNearestDeadlineFirst(self):
        response = self.app.get(reverse('jobs-list'))
